...
// - Légendes texte seul en bas à droite
// -----------------------------------------------------------
...
function createPowerPoint() {
  ...
  function addCoverSlide() {
    ...
    slide.addText(`Client : ${clientName}`, {
      x:0.5, y:0.5, fontSize:30, color:"FFFFFF", bold:true
    });
    slide.addText("Projet d’infrastructure de recharge", {
      x:0.5, y:1.3, fontSize:20, color:"FFFFFF"
    });
  }
  ...
  // ---------------- Légende (texte seul, sans encadré) ----------------
  ...
  function placeImageAndShapes(slide, title, imgBox, dataUrl) {
    ...
    // --------- PLAN D'IMPLANTATION : rouge + cercle vert + légende
  ...
  function addChecklistSlides() {
    ...
    items.forEach(item => {
      ...
      slide.addText(item.title, {
        x:MARGIN, y:0.5, fontSize:24, bold:true
      });
      ...
      if (file?.files.length > 0) {
        ...
        reader.onload = e => next(e.target.result);
